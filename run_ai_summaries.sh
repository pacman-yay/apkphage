...
# Reads a work/<SampleName>/llm_stage.json, calls Gemini via ai_summarizer.py,
# prints per-file + final summaries to the terminal, and merges the structured
# LLM output into work/<SampleName>/report.json.
#
# One-time setup is automatic: creates ./.venv and installs google-genai.
#
# Usage:
...
if [ -z "${GEMINI_API_KEY:-}" ]; then
    echo "[-] GEMINI_API_KEY is not set. Export it first:" >&2
...
if [ ! -x ".venv/bin/python" ]; then
    echo "[*] Setting up .venv and installing google-genai (one-time)..."
    python3 -m venv .venv
    .venv/bin/pip install -q google-genai
fi
PYTHON=".venv/bin/python"
if ! "$PYTHON" -c "import google.genai" 2>/dev/null; then
    echo "[*] Installing google-genai into .venv..."
    .venv/bin/pip install -q google-genai
fi

echo "[*] Summarizing $(basename "$(dirname "$STAGE")") via Gemini..."
